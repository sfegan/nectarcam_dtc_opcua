...
def mock_hal():
    ...
    with patch.multiple(
        'l2trig_low_level',
        get_power_enable=Mock(return_value=0xFFFE),
        set_power_enable=Mock(),
        get_power_current=Mock(return_value=100.0),
        get_ctdb_firmware_revision=Mock(return_value=0x0123),
        get_over_current_errors=Mock(return_value=0x0000),
        get_under_current_errors=Mock(return_value=0x0000),
        get_power_current_min=Mock(return_value=100),
        get_power_current_max=Mock(return_value=2000),
        set_power_channel_enable=Mock(),
        set_power_enable_all=Mock(),
    ) as mocks:
        yield mocks
...
class TestCTDBController:
    ...
    @pytest.mark.asyncio
    async def test_get_status(self, ctdb_controller, mock_hal):
        """Test getting CTDB status"""
        status = await ctdb_controller.get_status()
    ...
    @pytest.mark.asyncio
    async def test_set_channel_power(self, ctdb_controller, mock_hal):
        """Test enabling/disabling a channel"""
        await ctdb_controller.set_channel_power(5, True)
        mock_hal['set_power_channel_enable'].assert_called()
        
        # Test invalid channel
        with pytest.raises(ValueError):
            await ctdb_controller.set_channel_power(20, True)
    
    @pytest.mark.asyncio
    async def test_set_all_channels(self, ctdb_controller, mock_hal):
        """Test enabling/disabling all channels"""
        await ctdb_controller.set_all_channels(True)
        mock_hal['set_power_enable'].assert_called_with(1, 0xFFFE, 1000)
        
        await ctdb_controller.set_all_channels(False)
        mock_hal['set_power_enable'].assert_called_with(1, 0x0000, 1000)
    
    @pytest.mark.asyncio
    async def test_set_channels_multiple(self, ctdb_controller, mock_hal):
        """Test setting multiple channels at once"""
        channel_states = {1: True, 3: False, 5: True}
        await ctdb_controller.set_channels(channel_states)
        
        mock_hal['set_power_enable'].assert_called()
    
    @pytest.mark.asyncio
    async def test_set_current_limits(self, ctdb_controller, mock_hal):
        ...
        with patch('l2trig_low_level.set_power_current_min') as mock_min, \
            ...
            await ctdb_controller.set_current_limits(100.0, 2000.0)
...
class TestL2TriggerSystem:
    ...
    @pytest.mark.asyncio
    async def test_initialization(self, l2_system):
    ...
    @pytest.mark.asyncio
    async def test_get_all_status(self, l2_system, mock_hal):
        """Test getting status of all boards"""
        all_status = await l2_system.get_all_status()
        
        assert len(all_status) == 3
        assert all(isinstance(s, CTDBStatus) for s in all_status.values())
    
    @pytest.mark.asyncio
    async def test_get_slot_status(self, l2_system, mock_hal):
        """Test getting status of specific slot"""
        status = await l2_system.get_slot_status(1)
        ...
        # Test invalid slot
        with pytest.raises(ValueError):
            await l2_system.get_slot_status(99)
    
    @pytest.mark.asyncio
    async def test_set_all_power(self, l2_system, mock_hal):
        """Test setting all power"""
        await l2_system.set_all_power(True)
        # Should call set_all_channels for each CTDB
        
    @pytest.mark.asyncio
    async def test_emergency_shutdown(self, l2_system, mock_hal):
        """Test emergency shutdown"""
        await l2_system.emergency_shutdown()
        mock_hal['set_power_enable_all'].assert_called_with(False, 1000)
    
    @pytest.mark.asyncio
    async def test_health_check(self, l2_system, mock_hal):
        """Test system health check"""
        health = await l2_system.health_check()
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_power_cycle(self, l2_system, mock_hal):
        """Test complete power cycle"""
        # Get initial status
        initial_status = await l2_system.get_slot_status(1)
        
        # Turn off all channels
        await l2_system.ctdbs[1].set_all_channels(False)
        
        # Turn on specific channel
        await l2_system.ctdbs[1].set_channel_power(5, True)
        
        # Get final status
        final_status = await l2_system.get_slot_status(1)
    
    @pytest.mark.asyncio
    async def test_error_detection(self, mock_hal):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_access(self, l2_system, mock_hal):
...
class TestPerformance:
    ...
    @pytest.mark.asyncio
    async def test_status_read_time(self, ctdb_controller, mock_hal):
        """Test that status reads are reasonably fast"""
        import time
        
        start = time.time()
        await ctdb_controller.get_status()
    ...
    @pytest.mark.asyncio
    async def test_parallel_reads(self, l2_system, mock_hal):
        """Test parallel reads from multiple boards"""
        import time
        
        start = time.time()
        await l2_system.get_all_status()
        duration = time.time() - start
