...
void print_help() {
    ...
    printf("  mcf <0|1>                  - Enable/Disable MCF trigger propagation\n");
    printf("  get_mcf                    - Read MCF threshold, delay and deadtime\n");
    printf("  mcf_thresh <val>           - Set MCF threshold (0-511)\n");
    printf("  mcf_delay <val>            - Set MCF delay (0-15, in 5ns steps)\n");
    printf("  deadtime <val>             - Set L1 deadtime (0-255, in 5ns steps)\n");
    printf("  slot_rev <slot>            - Read CTDB firmware revision for a slot\n");
    printf("  pwr_all <0|1>              - Set power to all CTDBs\n");
    printf("  pwr_slot <slot> <hex_mask> - Set power mask for a slot (16-bit hex)\n");
    printf("  pwr_ch <slot> <ch> <0|1>   - Set power for a channel (1-15)\n");
    printf("  curr <slot> <ch>           - Read current for a channel (0=CTDB, 1-15=channel)\n");
    printf("  trig_slot <slot> <hex_mask>- Set trigger mask for a slot (16-bit hex)\n");
    printf("  trig_ch <slot> <ch> <0|1>  - Set trigger for a channel (1-15)\n");
    printf("  trig_delay <slot> <ch> <v> - Set trigger delay (0-255, in 37ps steps)\n");
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        if (strcmp(cmd, "quit") == 0 || strcmp(cmd, "exit") == 0) {
            ...
            } else printf("Usage: mcf <0|1>\n");
        } else if (strcmp(cmd, "get_mcf") == 0) {
            ...
            int slot;
            unsigned int mask;
            if (sscanf(line, "%*s %d %x", &slot, &mask) == 2) {
            ...
            int slot, ch, val;
            if (sscanf(line, "%*s %d %d %d", &slot, &ch, &val) == 3) {
